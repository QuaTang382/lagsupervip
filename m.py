...
def add_admin(message):
    ...
    if user_id in super_admins:   # chỉ super admin mới thêm được
...
def set_time_for_target(message):
    ...
    user_attack_data[message.chat.id] = (ip, port, time_val)
...
def handle_photo(message):
    try:
        ...
        if match:
            ...
            bot.reply_to(message, f"✅ Tìm thấy target: {ip}:{port}\nVui lòng nhập time (≤180 giây):")
        else:
...
def save_target(message):
    ...
    user_attack_data[message.chat.id] = (ip, port, attack_time)
    bot.reply_to(
        message,
